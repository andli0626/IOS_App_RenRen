...
static RRWebImageCache *instance;

@implementation RRWebImageCache
...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        _diskCachePath = [[[paths objectAtIndex:0] stringByAppendingPathComponent:@"ImageCache"] retain];
...
- (void)dealloc
{
    RL_RELEASE_SAFELY(_memCache);
    RL_RELEASE_SAFELY(_diskCachePath);
    RL_RELEASE_SAFELY(_cacheInQueue);
...
+ (RRWebImageCache *)sharedImageCache
{
    if (instance == nil)
    {
        instance = [[RRWebImageCache alloc] init];
    }
...
- (void)notifyDelegate:(NSDictionary *)arguments
{
    ...
    id <RRWebImageCacheDelegate> delegate = [arguments objectForKey:@"delegate"];
...
- (void)queryDiskCacheOperation:(NSDictionary *)arguments
{
    ...
    UIImage *image = RRScaledImageForPath(key, [NSData dataWithContentsOfFile:[self cachePathForKey:key]]);
...
- (void)storeImage:(UIImage *)image imageData:(NSData *)data forKey:(NSString *)key toDisk:(BOOL)toDisk
{
    ...
    if (toDisk)
    {
        ...
        NSInvocationOperation *operation = [[[NSInvocationOperation alloc] initWithTarget:self selector:@selector(storeKeyWithDataToDisk:) object:keyWithData] autorelease];
...
- (void)queryDiskCacheForKey:(NSString *)key delegate:(id <RRWebImageCacheDelegate>)delegate userInfo:(NSDictionary *)info
{
    ...
    NSInvocationOperation *operation = [[[NSInvocationOperation alloc] initWithTarget:self selector:@selector(queryDiskCacheOperation:) object:arguments] autorelease];
    [_cacheOutQueue addOperation:operation];
}
...
- (void)clearMemory
{
    [_cacheInQueue cancelAllOperations]; // won't be able to complete
