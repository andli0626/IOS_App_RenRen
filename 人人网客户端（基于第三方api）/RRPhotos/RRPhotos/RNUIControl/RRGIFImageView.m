...
- (void)setGIF_frames:(NSMutableArray *)gifFrames {
	...
	//NSLog(@"!!!!!!start time :%@",[NSNumber numberWithDouble:[[NSDate date] timeIntervalSince1970]]);
	[self loadImageData];
    //NSLog(@"!!!!!!end time :%@",[NSNumber numberWithDouble:[[NSDate date] timeIntervalSince1970]]);
}
...
- (void)loadImageData {
	...
	NSMutableArray *overlayArray = [[[NSMutableArray alloc] init] autorelease];
...
{
    NSMutableArray * gifFrames = [[RRGIFImageView getGifImageFrameFromMem:fileName] retain];
...
+(void)clearGifImageFrameMemStoreifOver:(NSInteger)maxSize
{
    if (ChatGifFrameCacheMemStore) {
        ...
        if (maxSize == 0) {
            [ChatGifFrameCacheMemStore release];
        }
    }
