...
@synthesize topPoint=_topPoint;
@synthesize leftPoint=_leftPoint;
    ...
    [op onCompletion:^(MKNetworkOperation *completedOperation)
     {
         ...
         if (netimage) {
             ...
             if(_canSetPosition  ){
                 ...
                 //做容错处理防止后面背景没有图片显示
                 if (curr_Y < self.bounds.size.height-height) {
                     curr_Y = self.bounds.size.height-height;
                 }
                 if (curr_Y > 0) {
                     curr_Y = 0;
                 }
                 ...
                 [self.imageView setNeedsLayout];
                 _canSetPosition=NO;
                 _leftPoint = fabsf(curr_X/self.imageView.frame.size.width);
                 _topPoint = fabsf(curr_Y/self.imageView.frame.size.height);
             }
         }
     }
...
-(void)moveToX:(CGFloat)x ToY:(CGFloat)y
{
    ...
    NSLog(@"syp===touchesMoved=%f,y=%f",_leftPoint,_topPoint);
}
          ...
          withEvent:(UIEvent *)event
{
    ...
    NSLog(@"syp===touchesMoved=%f,y=%f",offsetX,offsetY);
    if(fabsf(offsetY)>=2)
    {
